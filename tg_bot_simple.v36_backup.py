...
BOT_VERSION = "tgbot-3.6-super-tradesR"
...
async def cmd_panel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    kb = InlineKeyboardMarkup([[stop_button], [restart_button]])
...
# ---------------------------------------------------------------------------
# /trades — СДЕЛКИ HUNTERS (БЕЗ MARKDOWN)
# ---------------------------------------------------------------------------

async def cmd_trades(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    Показ сделок HUNTERS с подсчётом суммарного PnL, win-rate, R-multiple и кратким списком.
    """
    if not await guard(update, context):
        return
    ...
    try:
        ...
        lines.append("━━━━━━━━━━━━━━━━━━")
        lines.append(f"Всего записей: {len(trades)}")
        lines.append(f"Открытых: {len(open_trades)} | Закрытых: {len(closed_trades)}")

        if closed_trades:
            win_rate = wins / len(closed_trades) * 100.0
            lines.append(f"Win rate (закрытые): {win_rate:.1f}%  (win={wins}, loss={losses})")
        else:
            lines.append("Win rate (закрытые): n/a")

        lines.append(f"Суммарный реализованный PnL: {total_realized_pnl:+.2f} USDT")
        lines.append("")
        ...
        # Закрытые сделки (с R-multiple и временем)
        if closed_trades_sorted:
            ...
            for t in closed_trades_sorted[:5]:
                ...
                # R-multiple
                risk_val_raw = t.get("risk_usd", t.get("planned_risk_usd", None))
                if risk_val_raw not in (None, 0, 0.0, "0", "0.0"):
                    try:
                        risk_val = float(risk_val_raw)
                        if risk_val != 0:
                            r_mult = pnl_val / risk_val
                            r_str = f", R={r_mult:+.2f}"
                        else:
                            r_str = ""
                    except Exception:
                ...
                # времена
                open_ts = t.get("entry_ts") or t.get("open_ts") or t.get("ts")
                close_ts = t.get("close_ts", 0)

                if isinstance(open_ts, (int, float)) and open_ts > 0:
                    open_str = datetime.fromtimestamp(open_ts).strftime("%H:%M")
                else:
                    open_str = "--:--"

                if isinstance(close_ts, (int, float)) and close_ts > 0:
                    close_str = datetime.fromtimestamp(close_ts).strftime("%H:%M")
                else:
                    close_str = "--:--"

                time_part = f"{open_str}→{close_str}"

                prof_suffix = f", {profile}" if profile else ""
                icon = "✅" if pnl_val >= 0 else "🔻"

                lines.append(
                    f"{icon} {time_part} {symbol} {side} {pnl_str} USDT ({reason}{prof_suffix}{r_str})"
                )

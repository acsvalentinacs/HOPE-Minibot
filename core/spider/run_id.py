...
# Purpose: Spider run_id generation with strict schema (fail-closed)
# === END SIGNATURE ===
"""
Spider Run ID Module

Generates unique run_id per spider execution with strict schema.

Format: spider_v1__ts=<UTC>__pid=<PID>__nonce=<NONCE_HEX>
...
Example:
    spider_v1__ts=20260125T164500Z__pid=92396__nonce=9f2c0d7e3a6b4c1d8a9e0f1b2c3d4e5f
...
# Strict regex for run_id validation
RUN_ID_REGEX = re.compile(
    r'^spider_v1__ts=(\d{8}T\d{6}Z)__pid=([1-9]\d{0,6})__nonce=([0-9a-f]{32})$'
)
...
def generate_run_id() -> str:
    ...
    try:
        ...
        # Format run_id
        run_id = f"spider_v1__ts={ts_str}__pid={pid}__nonce={nonce}"

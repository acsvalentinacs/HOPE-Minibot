...
REM ============================================================
REM Send message to Claude via Friend Bridge (through SSH tunnel)
REM
REM Usage: send_to_claude.cmd "Your message here"
REM Requires: friend_chat_tunnel.cmd running in another window
REM ============================================================

if "%~1"=="" (
    echo Usage: send_to_claude.cmd "Your message"
    echo.
    echo Example: send_to_claude.cmd "Fix the bug in ipc_agent.py"
    exit /b 1
)
...
echo Sending to Claude: %MESSAGE:~0,50%...
echo.

curl -s -X POST "%BRIDGE_URL%/send" ^
    -H "Content-Type: application/json" ^
    -H "X-HOPE-Token: %FRIEND_BRIDGE_TOKEN%" ^
    -d "{\"to\": \"claude\", \"message\": \"%MESSAGE%\", \"context\": \"friend_chat\"}"
